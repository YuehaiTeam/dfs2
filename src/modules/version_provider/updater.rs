...
use super::{PluginVersionProvider, VersionCache};
...
impl VersionUpdater {
            ...
            loop {
                ...
                match self.update_versions().await {
                    Ok(update_count) => {
                        if update_count > 0 {
                            debug!(
                                "Version update cycle completed, updated {} resources",
                                update_count
                            );
                        }
                    }
    ...
    async fn update_versions(&self) -> Result<usize, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let resources_to_check: Vec<(String, u32)> = config
            ...
            .filter_map(|(resource_id, resource)| {
                resource
                    .version_provider
                    .as_ref()
                    .map(|vp| (resource_id.clone(), vp.cache_ttl.unwrap_or(300)))
            })
            .collect();
        ...
        for (resource_index, (resource_id, cache_ttl)) in resources_to_check.into_iter().enumerate()
        {
            ...
            if self
                .version_cache
                .should_update(&resource_id, cache_ttl)
                .await
            {
        ...
        if update_count > 0 {
            info!(
                "Version update cycle completed: {} resources updated out of {} checked",
    ...
    async fn update_resource_version(
        ...
        match self.plugin_provider.fetch_version_info(resource_id).await {
            Ok(version_info) => {
                ...
                // 检查版本是否实际发生了变化
                let version_changed = current_cached
                    .as_ref()
                ...
                if let Err(e) = self
                    .version_cache
                    .set_cached_version_info(resource_id, &version_info, cache_ttl)
                    .await
                {
    ...
    pub async fn update_resource_immediately(
        &self,
        resource_id: &str,
    ) -> crate::error::DfsResult<String> {
        ...
        let version_provider = resource.version_provider.as_ref().ok_or_else(|| {
            crate::error::DfsError::invalid_config(
        ...
        info!(
            "Immediately updated version for {}: {}",
...
pub async fn initialize_version_system(
    ...
    for (resource_id, resource) in &config.resources {
        if let Some(version_provider) = &resource.version_provider {
            // 检查缓存是否已存在
            if version_cache
                .get_cached_version(resource_id)
                .await
                .is_none()
            {
                let cache_ttl = version_provider.cache_ttl.unwrap_or(300);
                ...
                // 使用latest字段作为初始缓存
                if let Err(e) = version_cache
                    .set_cached_version(resource_id, &resource.latest, cache_ttl)
                    .await
                {
                ...
                info!(
                    "Set initial version cache for {}: {}",
                    resource_id, resource.latest
                );
                ...
                tokio::spawn(async move {
                    match plugin_provider.fetch_version_info(&resource_id).await {
                        Ok(version_info) => {
                            ...
                            {
                                error!(
                                    "Failed to initialize version cache for {}: {}",
                                    resource_id, e
                                );
                            } else {
                                info!(
                                    "Initialized version for {}: {}",
                                    resource_id, version_info.version
                                );
                            }
                        }
