...
impl GitLfsSigner {
    pub fn from_url(url_str: &str) -> Result<Self> {
        ...
        // 自动拼接LFS端点
        let lfs_endpoint = format!("{repo_url}/info/lfs");
        ...
        debug!(
            "Created GitLfsSigner: repo={}, lfs={}, raw={}",
    ...
    pub async fn generate_url(
        ...
        let (sha256, size) = if let Ok(Some(cached)) = data_store.get_string(&cache_key_file).await
        {
            ...
            if parts.len() == 2 {
                ...
                self.fetch_lfs_pointer_and_cache(path, &cache_key_file, data_store)
                    .await?
            }
        } else {
            self.fetch_lfs_pointer_and_cache(path, &cache_key_file, data_store)
        ...
        // 缓存下载URL
        let _ = data_store
            .set_string(&cache_key_url, &download_url, Some(self.url_cache_ttl))
            .await;
        debug!(
            "Cached download URL for SHA256: {} -> {}",
            sha256, download_url
        );
    ...
    async fn fetch_lfs_pointer_and_cache(
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!(
                "Failed to fetch LFS pointer from {}: {}",
                raw_url,
                response.status()
            ));
        }
        ...
        let sha256 = pointer
            .oid
            .strip_prefix("sha256:")
            .unwrap_or(&pointer.oid)
            .to_string();

        // 缓存文件→SHA256映射
        let cache_value = format!("{}:{}", sha256, pointer.size);
        let _ = data_store
            .set_string(cache_key, &cache_value, Some(self.file_cache_ttl))
            .await;
    ...
    fn parse_lfs_pointer(&self, content: &str) -> Result<LfsPointer> {
        ...
        if oid.is_empty() || size == 0 {
            return Err(anyhow::anyhow!(
                "Invalid LFS pointer format: missing oid or size"
            ));
        }
    ...
    fn test_git_lfs_url_parsing_missing_raw() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Missing 'raw' parameter")
        );
    }
...
size 12345"#;
        ...
        let signer =
            GitLfsSigner::from_url("https://github.com/test/test.git?raw=https://example.com/")
                .unwrap();
    ...
    fn test_lfs_pointer_parsing_invalid() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Invalid LFS pointer format")
        );
    }
